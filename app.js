...
let LOG_KEY = 'secureChatLog_default';    // will be overwritten after login
...
/* Safety guard – make sure LOG_KEY is always defined */
function ensureLogKey() {
  if (!LOG_KEY) LOG_KEY = 'secureChatLog_default';
}
...
/* ---------- Login button ----------
   Compute a unique LOG_KEY, load its history, then show the chat UI. */
loginBtn.addEventListener('click', async () => {
  const pw = loginPw.value.trim();
  if (!pw) return alert('Please type a passphrase');

  activePassphrase = pw;
  hiddenPw.value = pw;                     // store for encrypt/decrypt calls
  LOG_KEY = await storageKeyFor(pw);       // ← unique key per passphrase
  renderLog();                             // load any existing messages
  showChat();
});
